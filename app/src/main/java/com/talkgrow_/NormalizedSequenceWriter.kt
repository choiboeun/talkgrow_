// android_app/src/main/java/com/talkgrow_/NormalizedSequenceWriter.kt
...
/**
 * 확정된 세그먼트의 [91 x 134] 시퀀스를 NPY(float32, shape=(91,134))로 저장.
 * 파일명: seg_<segmentId>_norm_t91_f134.npy
 */
class NormalizedSequenceWriter(
    context: Context,
    ...
    fun appendSequence(segmentId: Long, sequence: Array<FloatArray>) {
        val t = sequence.size
        val f = if (t > 0) sequence[0].size else 134
        val file = File(outDir, "seg_${segmentId}_norm_t${t}_f${f}.npy")
